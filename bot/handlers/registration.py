...
async def register_command(message: Message, state: FSMContext, bot_username: str) -> None:
    ...
    await message.answer("Введите ваш player tag (например #ABC123):", reply_markup=main_menu_reply())
...
async def register_token(
    ...
    if not player_tag:
        await state.clear()
        await message.answer("Что-то пошло не так. Начните /register заново.", reply_markup=main_menu_reply())
        return
    ...
    try:
        ...
        await message.answer("Не удалось проверить токен. Попробуйте позже.", reply_markup=main_menu_reply())
        return

    if not verified:
        await message.answer("Токен не подходит. Проверьте правильность.", reply_markup=main_menu_reply())
        return
    ...
    try:
        ...
        await message.answer("Не удалось получить профиль игрока.", reply_markup=main_menu_reply())
        return

    clan_tag = player_data.get("clan", {}).get("tag")
    if not clan_tag or clan_tag.upper() != config.clan_tag.upper():
        await message.answer("Игрок не состоит в этом клане.", reply_markup=main_menu_reply())
        return
    ...
    await state.clear()
    await message.answer("Регистрация завершена. Спасибо!", reply_markup=main_menu_reply())
